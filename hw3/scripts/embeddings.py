...
class Model:
    ...
    def find_analogy(self, word1, word1_prime, word2):
        ...
        word1 = self.embedding.Projection(word1)
        word1_prime = self.embedding.Projection(word1_prime)
        word2 = self.embedding.Projection(word2)
        words = self.vocab.words
        projections = map(self.embedding.Projection, words)
        values = [self.cosine(w, word1_prime) + self.cosine(w, word2) - self.cosine(w, word1) for w in projections]
        # values = [(self.cosine(w, word1_prime) + 1) * (1 + self.cosine(w, word2)) / (1 + 1e-20 + self.cosine(w, word1)) for w in projections]
        result = sorted(zip(values, words), reverse=True)[:self.n]
        return result
